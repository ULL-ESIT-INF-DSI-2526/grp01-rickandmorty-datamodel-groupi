...
describe("Clase Dimension", () => {
  ...
  test("Creación de instancia y getters básicos", () => {
    const dim = new Dimension(id1, "Dimensión Cronenberg", "Activa", 8, "Lugar de origen del Rick original");
    
    expect(dim.id).toBe(id1);
  ...
  test("Debería aceptar diferentes formatos de ID válidos", () => {
    const dim2 = new Dimension(id2, "Dimensión Doofus", "Activa", 5, "Hogar de Doofus Rick");
    const dim3 = new Dimension(id3, "Dimensión C-500A", "En cuarentena", 10, "Alta tecnología");
    
    expect(dim2.id).toBe("J19Z7");
    expect(dim3.id).toBe("C-500A");
  })

  test("Debería lanzar error si el ID no es válido", () => {
    // No empieza por letra, símbolos no válidos al inicio o vacío
  ...
  test("Setter de nivel_tecnologico: validación rango", () => {
